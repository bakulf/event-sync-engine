...
1. Fetch all `m_*` keys
2. Look for any device with available baseline (e.g., first device found, device A)
3. **If baseline found:**
...
5. Sort collected events by HLC
6. Apply events in HLC order
7. Initialize `known_increments` with all devices' `last_increment`
8. Initialize own `last_increment = 0` (next event will be increment 1)
9. Update local HLC: `max(Date.now(), max(all hlc_time from events))`
10. Create initial baseline with current `known_increments` as `includes`
11. Write to storage.sync: meta (with `last_increment = 0`, `shards = [0]`), baseline, seen vector `s_<own_UUID>`
...
5. Write to storage.sync:
   - `m_<UUID>` (with `last_increment = 0`, `shards = [0]`)
...
9. Update `m_<own_UUID>` with new shard list
